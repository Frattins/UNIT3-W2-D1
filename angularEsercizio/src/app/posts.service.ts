...
getRandomPosts(num: number): PostInterface[] {
  ...
  getActivePosts(): PostInterface[] {
    return this.posts.filter((post) => post.active === true);

}

getinactivePosts(): PostInterface[] {
  return this.posts.filter((post) => !post.active);

}

getUniqueTags(): string[] {
  //Uso `map` per ottenere un array di array di tag.
  const allTagsArrays = this.posts.map(post => post.tags);

  //Rendo l'array di array di tag in un singolo array contenente tutti i tag.
  const allTags = allTagsArrays.flat();

  // Utilizzo un `Set` per ottenere solo i tag unici.
...
getPostsByTag(tag: string): PostInterface[] {
